...
def reset():
    ...
    db=[]
    for ID in range(32):
        db.append([ID, random.normalvariate(0,6), [], [], [], 0, 0, [], 0, [0,0,0], [random.lognormvariate(0.75,0.75),random.lognormvariate(-1.35,1),random.lognormvariate(-1.35,1)], []])
# Player File: ID, rating, results, opponents, performances, average performance, wins, margins of victory, rating, injury length [QB, RB, WR], value of injured player [QB, RB, WR], injury per game 

def play(p1,p2):
    global db
    pA=random.normalvariate(db[p1][1],9)+1.5
    pB=random.normalvariate(db[p2][1],9)-1.5
    db[p1][11].append(0)
    db[p2][11].append(0)
    if db[p1][9][0]>0:
        db[p1][9][0]-=1
        pA-=db[p1][10][0]
        db[p1][11][-1]+=db[p1][10][0]
    if db[p2][9][0]>0:
        db[p2][9][0]-=1
        pB-=db[p2][10][0]
        db[p2][11][-1]+=db[p2][10][0]
    if db[p1][9][1]>0:
        db[p1][9][1]-=1
        pA-=db[p1][10][1]
        db[p1][11][-1]+=db[p1][10][1]
    if db[p2][9][1]>0:
        db[p2][9][1]-=1
        pB-=db[p2][10][1]
        db[p2][11][-1]+=db[p2][10][1]
    if db[p1][9][2]>0:
        db[p1][9][2]-=1
        pA-=db[p1][10][2]
        db[p1][11][-1]+=db[p1][10][2]
    if db[p2][9][2]>0:
        db[p2][9][2]-=1
        pB-=db[p2][10][2]
        db[p2][11][-1]+=db[p2][10][2]
    ...
    if random.random()<0.025:
        db[p1][9][0]+=round(random.lognormvariate(1.041,0.426))
    if random.random()<0.025:
        db[p2][9][0]+=round(random.lognormvariate(1.041,0.426))
    if random.random()<0.025:
        db[p1][9][1]+=round(random.lognormvariate(1.185,0.426))
    if random.random()<0.025:
        db[p2][9][1]+=round(random.lognormvariate(1.185,0.426))
    if random.random()<0.025:
        db[p1][9][2]+=round(random.lognormvariate(1.185,0.426))
    if random.random()<0.025:
        db[p2][9][2]+=round(random.lognormvariate(1.185,0.426))
...
def calcSimpleRatings():
    ...
    for i in range(10):
        ratings=[]
        for plr in db:
            ratings.append(sum([db[plr[3][opp]][8]+plr[7][opp]-db[plr[3][opp]][11][opp] for opp in range(16)])/16)
        for plr in range(32):
